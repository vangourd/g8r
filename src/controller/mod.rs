...
use crate::kv::{GlobalKvStore, StackContext, VariableContext};
...
pub struct Controller {
    ...
    global_kv: GlobalKvStore,
}

impl Controller {
    pub fn new(state: StateManager) -> Self {
        let global_kv = GlobalKvStore::new_with_database(state.clone());
        Self {
            state,
    ...
    pub fn new_with_global_kv(state: StateManager, global_kv: GlobalKvStore) -> Self {
        Self {
            state,
            modules: Arc::new(HashMap::new()),
            global_kv,
    ...
    /// New reconcile method with variable context support
    ...
    pub async fn reconcile_from_nickel_with_variables(&self, config_path: &str, stack_name: &str) -> Result<()> {
        ...
        // Create variable context for this stack
        let stack_context = StackContext::new(stack_name.to_string());
        let mut var_context = VariableContext::new(stack_name.to_string(), self.global_kv.clone());
        
        // Load initial rosters (these don't need variable context)
        ...
        // Load initial duties to get constants and dependencies
        let initial_duties = evaluator.load_duties()?;
        
        // Extract constants from initial config
        let mut constants = HashMap::new();
        for duty in &initial_duties {
            if let Some(metadata) = &duty.metadata {
                if let Some(const_values) = metadata.get("constants") {
                    if let Some(const_obj) = const_values.as_object() {
                        for (key, value) in const_obj {
        ...
        for (batch_idx, batch_names) in execution_plan.iter().enumerate() {
            ...
            // Re-evaluate duties with current variable context
            let current_duties = evaluator.load_duties_with_variable_context(&var_context).await?;
            ...
            for duty_name in batch_names {
                ...
                // Store outputs in stack context for next duties
                if let Some(outputs) = result_json.get("outputs") {
                    let duty_outputs_key = format!("duties.{}.outputs", duty.name);
                    var_context.stack_context.set_from_json(&duty_outputs_key, outputs)?;
                    info!("Stored outputs for duty '{}' in stack context", duty.name);
                }
            }
        ...
        info!("All {} batches completed successfully for stack '{}'", execution_plan.len(), stack_name);
        Ok(())
    }
